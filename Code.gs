...
var URL = "https://docs.google.com/forms/d/1Fb70JIKGymw67zmaLyotTZFny_rL6seUalbVtkvflYc/"; // dummy form
...
function onOpen() {
  ...
  ui.createMenu('Scripts')
    .addItem('Download Form as JSON', 'main')
    .addToUi();
}
...
function dlFile(blob) {
  let timestampString = new Date().toISOString().replace(/[-:_.]/g, '');
  let file = DriveApp.getRootFolder().createFile(`formExport-${timestampString}.json`, blob);
